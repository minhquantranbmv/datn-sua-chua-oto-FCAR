        ...
        <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control"
                    @if (!empty($_GET['search_booking'])) value={{ $_GET['search_booking'] }}
            @else
                value="" @endif
...
<div class="form-group">
    <select class="form-control" name="status" id="">
        ...
        <option @isset($_GET['status'])
            @if ($_GET['status']=='received')
                selected
            @endif
        @endisset value="received">Chưa xác nhận</option>

        <option @isset($_GET['status'])
        @if ($_GET['status']=='latch')
            selected
        @endif
    @endisset value="latch">Xác nhận</option>
        <option @isset($_GET['status'])
        @if ($_GET['status']=='cancel')
            selected
        @endif
    @endisset value="cancel">Hủy bỏ</option>
                            ...
                            <div class="form-group d-flex" width="50px">
                                ...
                                <form action="{{ route('dat-lich.chuyen-trang-thai') }}" method="POST" class="d-flex">
                                    ...
                                    <select class="form-control" name="status_booking" id="">
                                        <option @if ($b->status_booking == 'received') selected @endif value="received">Chưa xác
                                            nhận</option>
                                        <option @if ($b->status_booking == 'latch') selected @endif value="latch">Xác nhận
                                        </option>
                                        <option @if ($b->status_booking == 'cancel') selected @endif value="cancel">Hủy bỏ
                                        </option>
                    ...
                    @endif --}}
                            @can('edit-booking')
                                <a name="" id="" class="btn btn-primary"
                                    href="{{ route('dat-lich.chi-tiet', ['id' => $b->id]) }}" role="button">Chi tiết</a>
                                <a name="" id="" class="btn btn-info"
                                    @if ($b->status_booking != 'latch') style="display: none" @endif
                                    href="{{ route('dat-lich.tiep-nhan-xe', ['booking_detail_id' => $b->id]) }}"
                                    role="button">Tiếp
                                    nhận xe</a>
                            @endcan
